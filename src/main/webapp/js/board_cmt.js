$(function() {
			...
			success: function(param) {
				...
				$(param.list).each(function(index, item) {
					...
					if (param.user_num == item.user_num) {
						output += '<input type="button" data-boardcmtnum="' + item.boardcmt_num + '"value="수정" id="modify_btn" class="btn-cmt"  >';
						output += '<input type="button" data-boardcmtnum="' + item.boardcmt_num + '"value="삭제" id="cmt_delete_btn" class="btn-cmt"  >';
					}
					output += '</div>';
					output += '</div>';
					output += '</div></div>';
	...
	$(document).on('click', '#modify_btn', function() {
		let boardcmt_num = $(this).attr('data-boardcmtnum');
		...
		modifyUI += '	<input type="hidden" name="boardcmt_num" id="boardcmt_num" value="' + boardcmt_num + '">';
	...
	$(document).on('click', '#cmt_delete_btn', function() {
		...
		if (check) {
			$.ajax({
				url: 'boardDeleteCmt.do',
				type: 'post',
				data: { boardcmt_num: boardcmt_num },
				dataType: 'json',
				cache: false,
				timeout: 30000,
				success: function(param) {

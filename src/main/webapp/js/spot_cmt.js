		...
		$.ajax({
			...
			success:function(param){
				...
				$(param.list).each(function(index,item){
					let output ='<div class="user justify-content-btween d-flex">';
					...
					if(item.modify_date){
						output+='<p class="date" style="padding-left: 2px;">'+item.modify_date+'</p>';
					}else{
						output+='<p class="date" style="padding-left: 2px;">'+item.reg_date+'</p>';
					}
					output +='</div></div>';
					output +='<p class="comment" id="cmt_content">'+item.cmt_content+'</p>';
					output +='</div></div></div>';
					output +='<div align="right">'
					if(param.user_num==item.user_num){
						output +='<input type="button" data-spotcmtnum="'+item.spotcmt_num+'"value="수정" id="modify_btn" class="btn btn-primary">';
						output +='<input type="button" data-spotcmtnum="'+item.spotcmt_num+'"value="삭제" id="delete_btn" class="btn btn-secondary">';
